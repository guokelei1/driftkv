...
- `foundation_manifests.py`: causal request/snapshot manifests.
...
Formal experiments must use frozen manifests and release cutoffs. Do not fit catalog maps, feature transforms, candidates, or cohorts on future/qualification data. The current feedback access bounds are enforced by `YambdaScaleDataset`.
