...
后续计划重新训练所有模型并重做全部实验（含 Motivation/Insight）；新一轮规模初步决定见下节，数据划分和执行协议尚待确定。下文冻结资产、配置和诊断定义是历史开发参考，不是新一轮已冻结的执行方案；旧 Insight 依赖本轮不修复。

## 新一轮 Yambda 模型训练初步计划（2026-09-15）

本轮已建立独立的 [计划与记录入口](unified_training_2026_09/README.md)。后续规模、数据、训练及评价决定统一维护在 [本轮训练计划](unified_training_2026_09/plan.md)；本节保留 2026-09-15 初步讨论快照。

本轮先完成 Yambda，三档名称为 **Medium、Large、Max**。Medium 和 Large 参考既有训练逻辑；
Max 为新增的更深、更多用户数据的模型。用户已确认以下 Max 初步配置，本文仅记录计划，未授权启动长训练。

| 项目 | Medium（既有参考） | Large（既有参考） | Max（本轮初步确定） |
| --- | --- | --- | --- |
...
Medium/Large 列是重训参考，不代表本轮全部训练设置已冻结。后续计划和讨论统一使用上述三档名称。

### Max 数据与成本预估

- 5B 的 listens、likes、dislikes 已下载，记录见
  [下载清单](../data/raw/download_manifest_yambda5b_v1.json)。本地 Parquet 元数据分别为
  4,649,567,411、89,334,605、11,579,143 行；尚未完成本轮人口筛选、紧凑 item 映射和训练 manifests。
- 拟按固定、label-free UID 规则选择 20 万合格用户并保留完整历史；具体选择规则、时间窗口及划分待确定。
  以既有 Large 人口平均行为量外推，完整 listens 约 **11.3 亿**；若沿用初始 `[0,217天)`
  窗口，该窗口 listens 约 **7.6 亿**。这些是预算估算，实际值以 5B 处理后的统计为准。
- Listens 是历史行为量，不是监督训练样本数。沿用显式反馈二分类时，还需统计训练窗口内
  like/dislike 经去重、冲突处理及有效历史筛选后的样本数。
- 参数量待初始训练窗口内 item 词表确定。当前实现主要由 item embedding 占据参数，约为
  `词表大小 × hidden size`；不得用未来 item 拟合初始映射，也不按行为量线性外推词表。
- 保持 hidden、context、训练轮数和样本构造一致，按用户数近似样本量，Max 相对既有 Large
  的 HSTU 层内训练计算约为 `(200000 / 79681) × (16 / 10) ≈ 4.0 倍`。
  这不是实测墙钟耗时；词表、显存、通信和实际样本量可能改变耗时。此前 3 倍以内的预算目标
  不适用于当前 20 万用户配置，用户希望避免 10 倍以上的训练耗时。

下一步先统计 Max 人口、历史、有效监督样本及初始词表，再用短训练探针估算显存、吞吐和总耗时。
本轮版本窗口、种子、epoch 与准入/评价协议仍待确定；正式长训练沿用前瞻合同、资源估计、
通过 canary 和用户明确 launch 的要求。现有封存证据及未开放数据边界保持原样。
