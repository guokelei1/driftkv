...
| [DESIGN3_FUTURE_DIRECTION.md](DESIGN3_FUTURE_DIRECTION.md) | D3 Action-Aware Out-of-Core Pipeline；论文骨架已定，机制/协议/结果未冻结 |
...
和 row-sharded execution 将它兑现为 physical sparsity；D3 再在 K/V 超出 per-rank usable
HBM 时，用 WavePlan constraints 生成 capacity-bounded ResidencyPlan，组织
ordinary-DRAM→GPU→ordinary-DRAM 流水。D2/D3 都不重新选择 exact/compiled。
Communication-aware semantic selection 和 organic mixed-version controller 属于三个 design
之后的未来反馈层。Motivation 2/3 都不依赖 serving trace，synthetic lookup contention 只作为
supporting characterization。

2026-07-28 已将旧的 D2 中间稿和 D2/D3 混合讨论分别收敛到
`DESIGN2_FINAL_PLAN.md` 与当时的 D3 future 文档后删除。2026-07-30 将 D3 重新裁决为
Action-Aware Out-of-Core Pipeline，原 mixed-version program graph/controller 降为更远期反馈层。
随后增加四阶段执行文档，
