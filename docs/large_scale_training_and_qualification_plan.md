...
记录始于：2026-08-30；状态整理：2026-09-05。

状态：**Large D14 当前工作序列已冻结为原 V0–V3、V4@2.0、direct-child V5@2.0；五条相邻边 aggregate AUC 全正**

本文区分原一 epoch qualification 队列和后续 post-hoc epoch 对照。
当前模型应按第 9 节 canonical 指针读取；前面的一 epoch 配方、严格门和失败结果是历史记录，
不能与当前工作序列混用。原正式 Reuse/PRO 已取消，本文命令不是新的启动清单。
...
原架构合同排除了 `12L/H320`，也不根据 Large AUC 在架构间选择。只有主点出现预注册的 OOM、非有限训练、
显存/吞吐、checkpoint 或 state-I/O 物理失败时，才依次考虑 `8L/H320`、`8L/H256`。focused canary
已经通过，该轮没有触发 fallback。

## 2. 数据与原 qualification 矩阵
...
原版本矩阵固定如下；实际完成及停止范围见第 3 节：

| Branch | 训练增量 | Update/edge | 评测窗口 | 时间窗口覆盖（非任务完成状态） |
| --- | ---: | ---: | --- | --- |
| D7 | 7 天 | 10 | E7 | 10 条均完整 |
| D14 | 14 天 | 5 | E7、E14 | 5 条 E7 完整；前 4 条 E14 完整 |
| D14 v4→v5 | 14 天 | 第 5 条 | E14 | `[287,301)`；按统一 E14 口径报告，并保留实际请求数 |

原基础队列的 shared v0 只训练一次；D7 和 D14 分别从 v0 开始沿 direct-parent candidate chain 训练。
该队列每个 checkpoint 训练一个完整 pass，fresh AdamW，foundation LR `2e-4`，update LR `5e-5`。
原队列的 16 个正式 checkpoint 均保留：shared v0、D7 v1–v10、D14 v1–v5。
这不是当前 canonical V4/V5 的 epoch 设置，也不是包括 canary、epoch 对照在内的全部权重数量。
...
- 已运行 cell 的 raw score 都在 label join 前封存；完整矩阵报告保留已完成、未裁决和未运行状态，
  不把计划中的 20 格写成全部完成。

原 Large PRO 对照配置为 `repair_width=256, carriers=64, represented_mass=4`，即与 C1024 保持 1/4 repair
和 1/16 carrier 比例；理论计算为 Exact-All 的 **8.919%**，sidecar 为每用户 3,200 个 FP32 scalar
（12,800 bytes）。其正式质量评估原拟仅用于 D14，后来在首格前取消；该实现只在资源 canary
中被验证过，不能写成已完成的 Large 方法结果，也不是当前 EvoKV 的成本估计。
...
原 qualification 队列入口如下。它不包含后续 V4/V5 epoch 对照，也不能替代 canonical 模型指针。
命令只用于说明历史产物的来源；重新运行长任务仍需明确授权。
...
# 原正式长队列命令（历史记录，不是当前待办）
python scripts/run_yambda500m_large_qualification.py \
  --mode formal \
  --acknowledge-long-run RUN_LARGE_D7_D14_10L_H320
```

原 runner 每次只运行一个四 rank job，队列定义为 `shared v0 → D7 checkpoints → D14 checkpoints → all
Full/admission → stop`。实际停止范围是第 3 节的 18/20 个已裁决 Full-only cell，
不能从队列定义推断全部完成。完整 artifact 会核对 seal/hash 后跳过；存在不完整目录时停止审计，
不会覆盖或悄悄续写。结构化进度位于
...
I/O 可执行；它不是质量证据。正式结果的训练 seed 才是 repeat unit。
原任务按 prospective qualification 启动，但正式 PRO 质量评估并未执行；
后来的 canonical 模型链来自 post-hoc development，二者不能合称为当前方法的 scale qualification。
这些历史记录也不授权 serving promotion。
...
因此原一 epoch Large 队列显示 release training 大多数时候有效，但不是一条全正的稳定 release chain；D14
v3→v4 是真实的模型 endpoint/admission 问题，不应归因于 Reuse 或 PRO。

## 8. 已完成的 D14 v3→v4 epoch 对照
...
该对照已经完成，入口保留为 `scripts/run_yambda500m_large_v3_v4_epoch_sweep.py`。
当时的 focused canary 只验证 20-step 训练、四卡 checkpoint 和五模型联合 Full 推理的正确性/显存，
不读取质量；正式任务在 canary 通过后由用户独立授权。该完成记录不授权新的 epoch 扫描。
