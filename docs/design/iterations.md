...
- 第六次专家路线完成，Design 1基础阶段冻结：实际方法/算法/准备依赖写入论文main.tex引用的两个章节，设计依据表、三张图及完整基础质量表就绪。唯一新增机制对照复用原64拟合/128诊断UID，common fit64/held64和同仿射下层q；93.84s完成，常量残余0.8164%–2.9244%能量、仿射0.0023%–0.0087%（诊断组）。逐发布K归属保持原总量，3万完整native发布13.76%–14.89%。无新共享方法/人口质量，未编译、确认未读，停止本轮；不自动开启第二设计。见[定稿报告](../../results/design/analysis/base_method_final_01/report.md)。

- 第五次专家补充完成：以Reuse为0完成冻结三方法的算子/次数、校准依赖、发布/生命周期、人口阈值和敏感性核算。完整native在固定256校准及当前成熟trace下，3万/10万/100万生命周期增量约为Exact重算的14.95%/4.88%/0.99%；共享源/教师构建和回放主导K。保留native结构，未启动新拟合/人口前向/优化。原1.422倍是时间比，撤回据此判断算法不节省或必须删源状态。原质量不变，Exact配对差已补；确认未读，分析结束。见[独立FLOPs报告](../../results/design/analysis/native_flops_01/report.md)。

- 最新第四次专家路线已完成：4091成熟UID、四次独立相邻迁移、88,860条真实反馈，冻结C与两份256×16消融全报。native四边等权AUC改善+1.1190pp [ +0.0436, +1.8932 ]，但相对摘要/无源状态优势未确认，当前成本小计为Exact的1.422倍。基础质量有收益，机制必要性与低成本仍未完整证明；全部作业结束，确认未读。见[基础真实质量报告](../../results/design/analysis/native_base_quality4091_01_report/report.md)。

- 最新一轮已完成：第三次专家意见已持久化，native残余审查、B=64×16、C=256×16完成，A=64×64冻结。C/Reuse诊断MSE为0.0549/0.2357/0.0693/0.9829（M1/M3/M4/M5）；M5尾部未收敛，原拟合组仍有局部过修正，不进入6000、不扩人数或结构，全部作业结束。详见[独立三臂报告](../../results/design/analysis/native_coverage384_report_01/report.md)。

- 最新追加请求已完成：专家第二次补充已写入同一[路线文档](expert_route_2026-09-07.md#第二次专家补充共享修正空间与闭环误差归因验证)。13条层干预、冻结decoder自由编码、固定16查询检查及唯一native输入匹配原型全部结束。共享空间能表达有用修正，实际方法在第一层已有明显错误；native输入四边均优于同容量摘要近似，但M5仍不及Reuse，不启动6000、不再增加变体。结果见路线末节及[原型报告](../../results/design/analysis/mechanism_native_input192_report_01/report.md)。
- 探索状态：上一轮goal已按用户要求收束。随后提供的[专家路线](expert_route_2026-09-07.md)已完成本轮验证：查询留出、匹配二乘二与一次宽预算源诊断均结束。仿射表示有强留出证据，但共享预测未解决困难边；不进入新6000人评价，所有作业已结束，方法仍未通过质量与完整成本联合验证。
...
- 上轮最终实验：方案15四个适配目标M1/M3/M4/M5恢复96.60%/28.95%/90.17%/37.29%，M2为native No-op；同6000用户、136610条五阶段请求，893.09s完成。相对方案9两种设计的四条配对区间均含0，不宣布稳定提升。
...
## 2026-09-07：专家路线持久化与机制诊断启动

用户在上一轮收束后提供新专家意见并要求实验验证，明确M2继续排除。已保存
[专家路线](expert_route_2026-09-07.md)：先严格查询留出的仿射表示，再做源信息×监督
方式的二乘二，机制明确后才考虑6000开发质量；不恢复方案9–15横向搜索。

新增`diagnose_query_holdout.py`复用同64拟合UID、128已使用诊断UID及真实native谱系，
拟合/留出/重拟合item集合严格分离，64配对预算与额外1024教师分开。M2不造teacher
场景、不加载/拟合Translator，不计入任何指标，仍真实native写入。记录闭环/响应、
query秩与系数稳定性、value/key分解和时间取整重合，短gap用户不删掉。

首`mechanism_query_holdout_canary_01`在FP32大响应相消的代数检查上失败，退出1。
保留失败原样；把**代数审计**改用FP64，服务reader仍原FP32，没有放宽算子语义。
`mechanism_query_holdout_canary_02`通过，4拟合+4诊断用户、四目标、56.46s、3192MiB。
后启动完整`mechanism_query_holdout192_01`，预算1680s，GPU0直接监控，未超30分钟估计。

固定探针的`FunctionalSummary`最小增删原型和显式Kronecker参考检查分别通过。
`mechanism_factorial_canary_01`只做二乘二实现/资源检查，18.73s、3176MiB。
与查询canary的native/Exact误差记录元数据相同；batch8与batch4使logit MSE最大差
5.96e-8，不称逐元素相同；完整B统一batch4。正式B等待完整A支持后启动。
二乘二共享源PCA32、仿射输出与0.01正则，功能组增加固定M0响应探针；源扫描成本
单列，尚未接入完整在线writer成本验证，不宣称已是新可部署方法。

`mechanism_auc_pairs_01`只读既有方案9两对照与方案15逐请求，110.59s完成。包含并列
半分的全部正负对重建AUC，与原记录在1e-12内一致。跨UID配对占99.876%–99.952%；
M5方案15跨UID贡献AUC约+0.007384，同UID约+0.0000107（均相对Reuse）。原始JSON
保留Exact修复、方法恢复/损伤、margin分组，不引入用户选择器或改主指标。

**A完整结果。** `mechanism_query_holdout192_01`在1239.09s完成，峰值allocated17259MiB，
退出0。192 UID、M1/M3/M4/M5分别560/769/775/781场景。拟合、留出与重拟合item
完全分离；各边约86.9%–87.5%的留出唯一整数时间未参与拟合，其余取整重合，不能称
整个时间集合完全分离。64-query oracle在128诊断UID留出面板的logit MSE分别为
3.08e-6/2.38e-6/4.52e-5/2.74e-5，是Reuse的0.029%/0.035%/0.080%/0.097%。
真实连续cutover子集的对应比例0.029%/0.039%/0.103%/0.089%，不是靠邻边重置控制
才成立。64拟合UID也保留强oracle；M5残余中单UID占77.8%，但总残余仍为Reuse的0.479%。
额外1024教师并未一致改善64结果，不用更大预算冒充匹配证据。

query设计矩阵数值秩中位数33，但能量熵有效秩约2.35–2.54，条件数中位数约6500–6955。
在相同下层view/坐标重拟合64查询，系数相对MSE为1.63%/2.71%/6.95%/4.59%，
留出函数变化相对MSE仅7.37e-6/1.16e-5/2.90e-5/2.76e-5。支持检查中间监督几何，
不能单凭此认定方案15的唯一失败原因。

value/key代数分解也给出一个**不支持原猜测**的结果：按UID、层等权汇总的key项能量/
总响应变化能量约20.50%/17.95%/18.67%/20.16%，M3/M5没有统一更高。交叉项约
37%–39%，不可当独立因果贡献；局部层/用户差异未被排除，不写“困难边由key主导”的故事。

结果与配对UID统计见
[查询留出报告](../../results/design/analysis/mechanism_query_holdout192_report_01/report.md)。
表示有明确留出余量，因此按预定分支启动`mechanism_factorial192_01`：同64+128 UID，
batch4，GPU1，预算600s，正式比较四组；M2无任何适配/教师诊断行，确认仍未读。

**B及最终分支。** 率口径`mechanism_factorial192_01`于156.54s完成，17256MiB。
核对发现旧代码使用每事件响应率，不能直接对应专家实际aggregate响应公式。原结果
保留为归一化对照，补充`N²/mean_fit(N²)`权重同时作用于系数和响应两侧；没有只对
一个实验单元改变历史长度权重。显式加权设计矩阵参考测试通过，聚合canary20.61s完成。
`mechanism_aggregate_factorial192_01`于155.71s完成，17256MiB，native/Exact误差
记录与A全部元数据及数值一致。主比较功能/响应−均值/系数在M3/M4/M5留出诊断UID
的差值分别+0.006361/+0.188540/+0.161482，描述性配对95%区间均为正；M1跨0。
直接响应局部拟合改善没有变成稳健闭环改善，不能据此扩大到6000人。

按条件分支只放宽一次源/编码预算：`mechanism_rich_source_canary_01`16+4 UID，33.42s；
结合已通过的聚合公式canary，启动32固定M0探针、PCA128和同容量均值/响应对照，
未做新的网格或选择器。`mechanism_rich_source192_01`168.18s完成，19056MiB。
丰富输入四边诊断UID留出MSE为0.013823/0.014805/0.044202/0.148009，只有M4
相对同容量均值对照的区间完全支持改善。M3/M5仍约为Reuse的2.15/5.26倍。
典型拟合UID的留出误差显著小于诊断UID，但部分拟合UID仍有巨大误差，因此不能
简单归因于样本量。32探针数值秩31但未中心化能量秩中位数仅1.244，仍是特定源
观察/线性编码的限制；没有证明完整源KV不能预测，也不能称信息问题已被解决。

丰富辅助状态884784 bytes/UID，另有原writer/view；M5源投影与共享层参数合计
276478064 bytes。全部backfill/teacher/拟合账本保留，嵌套factorial_total不重复相加。
这是机制成本，未进行在线writer、人口I/O、等待期或6000质量，未更换基础kernel。
之前率口径key分解结论也用物理aggregate口径复核：key/总能量21.31%/19.06%/
19.54%/21.63%，仍不支持困难边整体更由key主导，交叉项另报。

最终判断、表格和证据指针写回[专家路线](expert_route_2026-09-07.md)末节。
M2在所有新结果中无适配/teacher诊断行；全保留真实M2 native谱系；M5 E14_partial。
本轮按预定判别停止，无新6000人或确认集输出、无论文编辑/编译，所有模型与统计作业
已结束。不把负结果扩大为整条路线不可行；后续应研究有效源观测、跨UID外推和响应
误差到闭环的放大机制，而不是继续加探针数或横向方案。
