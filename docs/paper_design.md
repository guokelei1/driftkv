...
完整目标 K/V 只作为 calibration 教师，正式评价用户不得进行逐用户目标拟合。
...
source 内容变化后刷新同用户剩余旧段的输出，因为转换使用跨段上下文。
...
生产读取路径和连续发布执行器仍待实现。具体协议见
[experimental_design.md](experimental_design.md)。新 calibration、连续迁移和长训练
仍需单独授权；清理仓库不会扩大执行范围。
