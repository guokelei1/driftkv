...
PLAN="configs/evokv_foundation/qk_theta2_negative_strength_two_gpu_v0.json"
ROUND_ROOT="results/foundation_model/qk_theta2/qk_theta2_negative_strength_20260807_round1"
...
CANDIDATES=(
    "theta2_strength_e2_lr075_n64"
    "theta2_strength_e2_lr075_n96"
    "theta2_strength_e3_lr075_n64"
    "theta2_strength_e2_lr100_n64"
)
ENTRIES=(
...
python scripts/materialize_evokv_qk_theta2_negative_strength.py \
...
if missing:
    raise FileNotFoundError(f"QK negative-strength inputs are absent: {missing}")
checks = (
...
if any(digest(path) != expected for path, expected in checks):
    raise RuntimeError("QK negative-strength frozen input differs")
if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
    raise RuntimeError("QK negative-strength round requires exactly GPU0/GPU1")
...
if output.is_file() and output.read_text() != payload:
    raise RuntimeError("QK negative-strength inputs changed after freeze")
...
if [[ "${EVOKV_PREFLIGHT_ONLY:-0}" == "1" ]]; then
    echo "QK theta2 negative-strength preflight completed"
    exit 0
fi
...
python scripts/summarize_evokv_qk_theta2_negative_strength.py \
...
if output.is_file() and output.read_text() != payload:
    raise FileExistsError("QK negative-strength return manifest differs")
output.write_text(payload)
(output.parent / "execution_complete.json").write_text(payload)
print(payload, end="")
PY

echo "QK theta2 negative-strength round completed: $RETURN_MANIFEST"
