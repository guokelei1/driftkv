...
SEEDS = (0, 1, 2, 3)
PROTOCOL = "cohort_tiered_migration_v1"
...
def complete(path: Path) -> bool:
    ...
    return (
        ...
        and result.get("study_stage") == "frozen_rule_replication"
        and result.get("selection", {}).get("fidelity_targets")
    )
...
def main() -> None:
    ...
    log_dir = Path("logs/cohort_tiered_migration_validation")
    ...
    for tier in TIERS:
        ...
        for seed in SEEDS:
            ...
            output = Path(
                ...
                f"{cell}_v2_cohort_tiered_seed{seed}.json"
            )
            ...
            command = [
                sys.executable,
                "scripts/cohort_tiered_migration_search.py",
                "--device",
                args.device,
                "--seed",
                str(seed),
