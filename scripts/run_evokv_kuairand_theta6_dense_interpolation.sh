...
config=configs/evokv_root_cause/kuairand_amplified_theta6_dense_interpolation_20260809_v0.json
protocol=configs/evokv_root_cause/kuairand_extended_triangle_acceptance_20260809_v0.json
selection_root=results/root_cause_campaign/kuairand_amplified_theta6_dense_interpolation_seed53117_v0
...
prefix_checkpoint_root=checkpoints/evokv_kuairand_amplified_fixed_seed53117_v0
assessment="$selection_root/theta6_extended_acceptance.json"

python -c 'from hstu_kvcache.streaming.kuairand_lineage_retrain import load_lineage_retrain_config; load_lineage_retrain_config("configs/evokv_root_cause/kuairand_amplified_theta6_dense_interpolation_20260809_v0.json")'
mkdir -p "$selection_root/logs" "$checkpoint_root"
test ! -e "$checkpoint_root/theta_6"
for version in 1 2 3 4 5; do
  checkpoint_target="$checkpoint_root/theta_${version}"
  if [[ ! -L "$checkpoint_target" ]]; then
    test ! -e "$checkpoint_target"
    ln -s "$PWD/$prefix_checkpoint_root/theta_${version}" "$checkpoint_target"
  fi
...

{
  ...
  find "$checkpoint_root" -maxdepth 1 -type l -printf '%f -> %l\n' | sort -V
