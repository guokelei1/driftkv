#!/usr/bin/env python3
"""Run the six frozen P11.1 population cells sequentially on GPU 0/1."""
...
OUTPUT = ROOT / "results/p11/p11_1_recursive_population_raw/full"
...
def run(job, device):
    ...
    command = ["python", "scripts/eval_p11_recursive_population.py", "--model", model,
               "--seed", str(seed), "--device", f"cuda:{device}"]
...
def main():
    ...
    if failures:
        raise SystemExit(f"P11.1 failed cells: {failures}")
