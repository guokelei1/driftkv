...
def _primary_gate(
    ...
    minimum_targets: int,
    anchor_recompute_ndcg: float,
    anchor_recompute_mrr: float,
    minimum_quality_ratio: float,
) -> dict[str, object]:
    ndcg = rolling_all["gaps"]["ndcg_at_10"]
    mrr = rolling_all["gaps"]["mrr"]
    relative = ndcg.get("relative_percent")
    recompute_ndcg = float(rolling_all["recompute"]["ndcg_at_10"])
    recompute_mrr = float(rolling_all["recompute"]["mrr"])
    ndcg_quality_ratio = recompute_ndcg / anchor_recompute_ndcg
    mrr_quality_ratio = recompute_mrr / anchor_recompute_mrr
    ...
    quality_floor_passed = (
        ndcg_quality_ratio >= minimum_quality_ratio
        and mrr_quality_ratio >= minimum_quality_ratio
    )
    passed = (
        target_passed
        and range_passed
    ...
    return {
        ...
        "minimum_anchor_recompute_quality_ratio": minimum_quality_ratio,
        "recompute_ndcg_at_10_anchor_ratio": ndcg_quality_ratio,
        "recompute_mrr_anchor_ratio": mrr_quality_ratio,
        "target_count_passed": target_passed,
        "relative_range_passed": range_passed,
        "joint_interval_passed": interval_passed,
        "fresh_quality_floor_passed": quality_floor_passed,
    }
...
def _row(
    ...
    result = json.loads(relevance_path.read_text())
    ...
    relation_gate = evaluation["primary_admission_gate"]
    return {
        "candidate": name,
        "origin": origin,
        "relevance_result": {
            "path": str(relevance_path),
            "sha256": file_sha256(relevance_path),
        },
        "training_result": {
        ...
        "training": {
            "edge": training["edge"],
            "parameter_probe": training["training"]["parameter_probe"],
            "diagnostic_checks": training["training"]["diagnostic_checks"],
            "optimizer_active_delta": training["training"][
        ...
        "rolling_all_gate": _primary_gate(
            ...
            relative_range,
            minimum_targets,
            anchor_recompute_ndcg,
            anchor_recompute_mrr,
            minimum_quality_ratio,
        ),
        "best_predeclared_relation": _best_checked(relation_gate["all_checked"]),
        "relation_admission_status": relation_gate["status"],
        "admitted_relations": relation_gate["admitted"],
    }
...
def summarize(plan_path: Path) -> dict[str, object]:
    ...
    minimum_quality_ratio = float(quality["minimum_anchor_recompute_quality_ratio"])
    anchor = plan["anchor_candidate"]
    anchor_recompute_ndcg = float(anchor["rolling_recompute_ndcg_at_10"])
    anchor_recompute_mrr = float(anchor["rolling_recompute_mrr"])
    rows = [
        _row(
    ...
    for candidate in plan["search_training"]["candidates"]:
        ...
        relevance_path = candidate_root / "relevance" / "result.json"
        training_path = candidate_root / "training" / "result.json"
        if not relevance_path.is_file() or not training_path.is_file():
            raise FileNotFoundError(f"QK core-heavy result is absent: {name}")
        rows.append(
            _row(
                name,
                candidate["axis"],
    ...
    return {
        "protocol": "evokv_qk_theta2_core_heavy_summary_v0",
        "status": (
        ...
        "interpretation_boundary": (
            "select at most one theta2 only after reviewing the rolling full-catalog joint NDCG/MRR gap gate and the fresh-quality floor; record-cluster intervals remain development diagnostics until seed replication"
        ),
    }
...
def _markdown(summary: dict[str, object]) -> str:
    ...
    for value in summary["candidates"]:
        ...
        gate = value["rolling_all_gate"]
        direct_kv = value["training"]["parameter_probe"].get("direct_kv")
        probe = "-" if direct_kv is None else f"{direct_kv['maximum_absolute_delta']:.6g}"
        lines.append(
            "| "
            + " | ".join(
                (
                    ...
                    f"{gate['recompute_ndcg_at_10_anchor_ratio']:.4f}",
                    f"{gate['recompute_mrr_anchor_ratio']:.4f}",
                    probe,
                    str(gate["status"] == "pass"),
                )
            )
            + " |"
        )
...
def main() -> None:
    ...
    for path, payload in payloads:
        if path.is_file() and path.read_text() != payload:
            raise FileExistsError(f"QK core-heavy summary differs: {path}")
