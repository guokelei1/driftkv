...
round_label="${1:-quality_chain_train8192_e1_round1}"
...
roles="configs/evokv_quality/qk_quality_roles_train8192_qual4096_v0.json"
edge_input="data/processed/evokv_quality/qk_xp_quality_train8192_qual4096_v0.npz"
edge_summary="configs/evokv_quality/qk_xp_quality_train8192_qual4096_v0_summary.json"
schedule="configs/evokv_quality/xp_qk_prequential_train8192_qual4096_e1_v0.json"
benchmark="configs/evokv_quality/x_qk_xp_quality_train8192_qual4096_v0.json"
...
static_inputs=(
  ...
  "scripts/run_evokv_quality_chain_retrain_round.sh"
)
...
minimum_disk_bytes=$(((150 + disk_floor_gib) * gib))
if (( disk_free_bytes < minimum_disk_bytes )); then
  echo "need 150 GiB candidate assets plus ${disk_floor_gib} GiB free floor" >&2
  exit 4
fi
...
if [[ -f "$static_hashes" ]]; then
  if ! cmp -s "$hash_candidate" "$static_hashes"; then
    echo "static round inputs changed; choose a new round label" >&2
    exit 5
  fi
else
...
value = {
    "schema": "evokv_qk_quality_chain_preflight_v0",
    "status": "pass",
    ...
    "retention": {
        "keep": ["theta1-theta3 checkpoints", "compact metrics", "bindings", "logs"],
...
python scripts/build_evokv_qk_quality_roles.py \
  ...
  --minimum-events 96 | tee "$log_root/build_roles.log"

if [[ -f "$edge_input" || -f "$edge_summary" ]]; then
  if [[ ! -f "$edge_input" || ! -f "$edge_summary" ]]; then
    echo "quality edge corpus is partial; remove or recover it explicitly" >&2
    exit 6
  fi
...
if (
    ...
    raise RuntimeError("existing quality edge corpus differs")
PY
  echo "reuse: validated quality edge corpus"
else
  python scripts/build_evokv_qk_xp_edge_inputs.py \
    --roles "$roles" \
    --output "$edge_input" \
    --summary "$edge_summary" \
    --hash-salt evokv-qk-quality-train8192-qual4096-v0 \
    --theta01-users 8192 \
    --theta12-users 2048 \
...
if schedule.epochs_per_update != 1 or len(schedule.updates) != 3:
    raise RuntimeError("generated quality schedule differs")
if inputs.benchmark["benchmark_id"] != "x_qk_xp_quality_train8192_qual4096_e1_two_gpu_development_v0":
    raise RuntimeError("generated quality benchmark differs")
PY
...
if (
    ...
    or len(result.get("updates", [])) != 3
    or any(not (root / f"theta_{version}" / "manifest.json").is_file() for version in (1, 2, 3))
):
    raise RuntimeError("quality chain training result differs")
PY
}
...
if [[ -f "$training_result" ]]; then
  ...
  validate_training
  echo "resume: validated training chain"
else
...
if (
    ...
    raise RuntimeError("quality chain diagnostic cell differs")
PY
}
...
for source_version in 0 1 2; do
  ...
  if [[ -f "$output" ]]; then
    if [[ "$resume" != "1" ]]; then
      echo "diagnostic cell exists without resume: $output" >&2
      exit 9
    fi
