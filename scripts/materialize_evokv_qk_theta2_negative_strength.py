...
PROTOCOL = "evokv_qk_theta2_negative_strength_round_plan_v0"
CANDIDATES = {
    "theta2_strength_e2_lr075_n64": (2, 1.125e-5, 1.125e-5, 1.125e-4, 64),
    "theta2_strength_e2_lr075_n96": (2, 1.125e-5, 1.125e-5, 1.125e-4, 96),
    "theta2_strength_e3_lr075_n64": (3, 1.125e-5, 1.125e-5, 1.125e-4, 64),
    "theta2_strength_e2_lr100_n64": (2, 1.5e-5, 1.5e-5, 1.5e-4, 64),
}
...
def _write_frozen(path: Path, value: dict[str, object]) -> None:
    ...
    if path.is_file():
        if path.read_text() != payload:
            raise FileExistsError(f"frozen QK negative-strength config differs: {path}")
        return
...
def _validate_plan(plan_path: Path, plan: dict[str, object]) -> None:
    ...
    anchor = plan.get("anchor_candidate")
    observed = {
        ...
        for value in plan.get("search_training", {}).get("candidates", [])
    }
    if (
        ...
        or anchor.get("candidate_name") != "theta2_relevance_e2_lr075_n32"
        or observed != CANDIDATES
    ):
        raise ValueError("QK theta2 negative-strength plan differs")
    source_directory = Path(source["root"]) / "theta_1"
    anchor_manifest = Path(anchor["checkpoint_root"]) / "theta_2" / "manifest.json"
    if (
        file_sha256(source_directory / "manifest.json")
        ...
        or file_sha256(anchor_manifest) != anchor["manifest_sha256"]
        or file_sha256(Path(anchor["training_result"]))
        != anchor["training_result_sha256"]
        or file_sha256(Path(anchor["relevance_result"]))
        != anchor["relevance_result_sha256"]
        or file_sha256(Path(plan["data"]["config"]))
        != plan["data"]["config_sha256"]
        or file_sha256(Path(plan["data"]["corpus"]))
        != plan["data"]["corpus_sha256"]
    ):
        raise ValueError(f"QK negative-strength frozen input differs: {plan_path}")
...
def _relevance_config(
    ...
    root = Path(plan["outputs"]["round_root"]) / "candidates" / candidate
    return {
        ...
        **_common(plan, candidate),
        ...
        "origin": {
            "kind": "negative_strength_search_candidate",
        ...
        "execution": {
            ...
            "snapshot_batch_size_per_rank": plan["execution"][
            ...
            "minimum_free_hbm_bytes_per_rank": plan["execution"][
                "minimum_free_hbm_bytes_per_rank"
            ],
            "minimum_free_disk_bytes": plan["execution"][
                "minimum_free_disk_bytes"
            ],
        },
        "outputs": {
            "round_root": str(root / "relevance"),
            "result": str(root / "relevance" / "result.json"),
        },
    }
...
def materialize(
    ...
    if only_candidate is not None and only_candidate not in CANDIDATES:
        raise ValueError(f"unknown QK negative-strength candidate: {only_candidate}")
    if canary_records_per_rank < 0:
        raise ValueError("QK negative-strength canary size differs")
    ...
    for candidate in plan["search_training"]["candidates"]:
        ...
        training_config = {
            ...
            **_common(plan, name),
            ...
            "training": {
                "objective": plan["search_training"]["objective"],
                "negative_count": candidate["negative_count"],
                "weight_decay": plan["search_training"]["weight_decay"],
                "seed": plan["search_training"]["seed"],
                "negative_seed": plan["search_training"]["negative_seed"],
                "optimizer_continuity": plan["search_training"][
            ...
            "execution": {
                ...
                "length_bucket_records": plan["execution"][
                    "length_bucket_records"
                ],
                "snapshot_batch_size_per_rank": plan["execution"][
                    "snapshot_batch_size_per_rank"
                ],
                "progress_every_steps": plan["execution"][
                    "progress_every_steps"
                ],
                "quality_progress_every_records": plan["execution"][
            ...
            "outputs": {
                ...
                "result": str(training_result),
                "metric_table_json": str(
                    candidate_root / "training" / "reuse_recompute_metrics.json"
                ),
                "metric_table_csv": str(
                    candidate_root / "training" / "reuse_recompute_metrics.csv"
                ),
                "metric_table_markdown": str(
                    candidate_root / "training" / "reuse_recompute_metrics.md"
                ),
            },
        }
