#!/usr/bin/env python3
"""Run the oracle attention-address signed-response coreset diagnostic."""
...
from insight_two.address_response_memory import (  # noqa: E402
...
CONTRACT = (
    ROOT
    / "configs/contracts/"
    "yambda500m_medium_hstu_native_insight2_address_response_coreset_v1.yaml"
)
OUTPUT_ROOT = RESULT_ROOT / "diagnostic_address_response_coreset_v1"
CANARY_USERS = 32
DISCOVERY_USERS = 512
FULL_COUNT = 1024
...
def verify_contract() -> tuple[dict[str, Any], str]:
    ...
    if contract["scope"]["edges"] != list(EDGES):
        raise RuntimeError("address-response edge order differs")
    quadrature = contract["address_quadrature"]
    if tuple(quadrature["sample_counts"]) != SUPPORTED_SAMPLE_COUNTS:
        raise RuntimeError("address-response sample-count grid differs")
    if int(quadrature["instrumentation_full_count"]) != FULL_COUNT:
        raise RuntimeError("address-response full instrumentation count differs")
    if tuple(contract["candidate_evaluation"]["heldout_indices"]) != HELDOUT_INDICES:
        raise RuntimeError("address-response heldout split differs")
    if contract["population"]["canary_indices_half_open"] != [0, CANARY_USERS]:
        raise RuntimeError("address-response canary scope differs")
    if contract["population"]["discovery_indices_half_open"] != [0, DISCOVERY_USERS]:
        raise RuntimeError("address-response discovery scope differs")
    execution = contract["execution"]
    if execution["GPUs"] != [0, 1, 2, 3]:
        raise RuntimeError("address-response GPU allowlist differs")
    if int(execution["canary_users"]) != CANARY_USERS:
        raise RuntimeError("address-response canary user count differs")
    if int(execution["discovery_users"]) != DISCOVERY_USERS:
        raise RuntimeError("address-response discovery user count differs")
    expected_output = OUTPUT_ROOT.relative_to(ROOT).as_posix()
    if contract["outputs"]["root"] != expected_output:
        raise RuntimeError("address-response output root differs")
    for record in contract["frozen_inputs"].values():
        path = ROOT / record["path"]
        if not path.is_file() or sha256_file(path) != record["sha256"]:
            raise RuntimeError(f"frozen address-response input differs: {path}")
...
def require_discovery_gate(contract_hash: str) -> None:
    ...
    if not analysis.is_file() or not estimate.is_file():
        raise RuntimeError(
            "address-response discovery requires canary adjudication and resources"
        )
    ...
    if gate.get("contract_sha256") != contract_hash or not gate.get(
        ...
        raise RuntimeError("address-response canary did not pass the launch gate")
    if resource.get("contract_sha256") != contract_hash:
        raise RuntimeError("address-response resource estimate differs")
    if float(resource.get("estimated_512_user_minutes", 1e9)) > 30:
        raise RuntimeError("address-response discovery exceeds 30 minutes")


def distributed_context() -> tuple[int, int, int]:
    if "RANK" not in os.environ:
        raise RuntimeError("address-response diagnostic requires torchrun")
    ...
    if world != 4:
        raise RuntimeError(
            f"address-response diagnostic requires four ranks, got {world}"
        )
    torch.cuda.set_device(local_rank)
    return rank, local_rank, world


def layer0_address_covering_radius(
    exact_cache,
    reuse_cache,
    positions: torch.Tensor,
) -> float:
    """Maximum nearest-landmark distance in normalized layer-0 address space."""

    features = torch.cat(
        (exact_cache.k[0, 0], reuse_cache.k[0, 0]), dim=-1
    ).to(dtype=torch.float32)
    features = F.normalize(features, p=2.0, dim=-1, eps=1e-12)
    centers = features.index_select(0, positions)
    squared = (features[:, None, :] - centers[None, :, :]).square().sum(dim=-1)
    return float(torch.sqrt(torch.min(squared, dim=1).values.max()))
...
def evaluate_user(
    *,
    ...
    records.append(
        {
            **common,
            ...
            "cluster_mass_min": 0,
            "cluster_mass_max": 0,
            "layer0_address_covering_radius": None,
            **metrics_row(score_metrics(exact_scores, reuse_scores, reuse_scores)),
        }
    )

    positions_by_count: dict[int, torch.Tensor] = {}
    audit: dict[str, Any] = {}
    ...
    for sample_count in SUPPORTED_SAMPLE_COUNTS:
        ...
        positions_by_count[sample_count] = memory.sample_positions.detach().cpu()
        masses = memory.inverse_inclusion_probabilities
        mass_sum = int(masses.long().sum().item())
        mass_min = int(masses.min().item())
        mass_max = int(masses.max().item())
        radius = layer0_address_covering_radius(
            exact_cache, reuse_cache, memory.sample_positions
        )
        audit[f"R{sample_count}_cluster_mass_sum"] = mass_sum
        audit[f"R{sample_count}_cluster_mass_min"] = mass_min
        audit[f"R{sample_count}_cluster_mass_max"] = mass_max
        audit[f"R{sample_count}_layer0_address_covering_radius"] = radius
        ...
        records.append(
            {
                **common,
                ...
                "cluster_mass_sum": mass_sum,
                "cluster_mass_min": mass_min,
                "cluster_mass_max": mass_max,
                "layer0_address_covering_radius": radius,
                **metrics_row(
                    score_metrics(exact_scores, reuse_scores, observed.scores)
                ),
            }
        )

    full_memory = build_oracle_address_response_memory(
        exact_cache, reuse_cache, sample_count=FULL_COUNT
    )
    full_positions = full_memory.sample_positions.detach().cpu()
    positions_by_count[FULL_COUNT] = full_positions
    full_masses = full_memory.inverse_inclusion_probabilities
    full_radius = layer0_address_covering_radius(
        exact_cache, reuse_cache, full_memory.sample_positions
    )
    audit["R1024_cluster_mass_sum"] = int(full_masses.long().sum().item())
    audit["R1024_cluster_mass_min"] = int(full_masses.min().item())
    audit["R1024_cluster_mass_max"] = int(full_masses.max().item())
    audit["R1024_layer0_address_covering_radius"] = full_radius
    ordered_counts = (*SUPPORTED_SAMPLE_COUNTS, FULL_COUNT)
    nested = all(
        torch.equal(
            positions_by_count[larger][:smaller], positions_by_count[smaller]
        )
        for smaller, larger in zip(ordered_counts, ordered_counts[1:])
    )
    mass_sums_valid = all(
        int(audit[f"R{count}_cluster_mass_sum"]) == exact_cache.seq_len
        for count in ordered_counts
    )
    masses_positive = all(
        int(audit[f"R{count}_cluster_mass_min"]) > 0 for count in ordered_counts
    )
    ...
    correctness = {
        ...
        "uid": uid,
        **audit,
        "nested_landmark_prefixes": nested,
        "all_cluster_mass_sums_equal_history_length": mass_sums_valid,
        "all_cluster_masses_positive": masses_positive,
...
def main() -> None:
    ...
    if rank == 0:
        ...
        atomic_json(
            partial / "configuration.json",
            {
                ...
                "selection_geometry": "normalized_layer0_Current_Parent_K",
            },
        )
    dist.barrier()
    ...
    for edge_index, edge in enumerate(EDGES):
        ...
        for local_offset, uid in enumerate(local_uids):
            items = torch.as_tensor(
                items_np[local_offset : local_offset + 1],
                dtype=torch.long,
                device=device,
            )
            actions = torch.as_tensor(
                actions_np[local_offset : local_offset + 1],
                dtype=torch.long,
                device=device,
            )
            deltas = torch.as_tensor(
                deltas_np[local_offset : local_offset + 1],
                dtype=torch.float32,
                device=device,
            )
            query_delta = torch.as_tensor(
                query_np[local_offset : local_offset + 1],
                dtype=torch.float32,
                device=device,
            )
        ...
        edge_records.append(
            {"edge": edge, "users": len(local_uids), "seconds": edge_seconds}
        )
        print(
            json.dumps(
                {
    ...
    if rank == 0:
        metrics = pd.concat(
            [
                pd.read_parquet(partial / f"rank{i}/metrics.parquet")
        ...
        expected_metrics = user_count * len(EDGES) * (
            1 + len(SUPPORTED_SAMPLE_COUNTS)
        )
        expected_correctness = user_count * len(EDGES)
        if len(metrics) != expected_metrics or len(correctness) != expected_correctness:
            raise RuntimeError("address-response raw row count differs")
        if metrics[["edge", "uid", "method"]].duplicated().any():
            raise RuntimeError("duplicate address-response metric key")
        maximum_logit_error = float(
            correctness.full_R1024_max_abs_logit_error.max()
        )
        maximum_readout_error = float(
            correctness.full_R1024_max_abs_readout_error.max()
        )
        passed = bool(
            ...
            and correctness.nested_landmark_prefixes.all()
            and correctness.all_cluster_mass_sums_equal_history_length.all()
            and correctness.all_cluster_masses_positive.all()
        )
        ...
        rank_summaries = [
            json.loads(
                (partial / f"rank{i}/summary.json").read_text(encoding="utf-8")
            )
            for i in range(world)
        ]
        artifacts = {
        ...
        summary = {
            "status": (
                "address_response_coreset_instrumentation_passed"
                if passed
                else "address_response_coreset_instrumentation_failed"
            ),
            "passed": passed,
            "contract_sha256": contract_hash,
            "scope": args.scope,
            ...
            "nested_landmark_prefixes": bool(
                correctness.nested_landmark_prefixes.all()
            ),
            "all_cluster_mass_sums_equal_history_length": bool(
                correctness.all_cluster_mass_sums_equal_history_length.all()
            ),
            "all_cluster_masses_positive": bool(
                correctness.all_cluster_masses_positive.all()
            ),
            "maximum_full_R1024_logit_error": maximum_logit_error,
            "maximum_full_R1024_readout_error": maximum_readout_error,
            "elapsed_seconds": max(
                float(row["elapsed_seconds"]) for row in rank_summaries
            ),
            "peak_allocated_mib": max(
