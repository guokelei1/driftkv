#!/usr/bin/env python3
"""Seal a conservative 512-user runtime estimate from the address canary."""
...
def main() -> None:
    ...
    if not run.get("passed") or not gate.get("discovery_launch_gate_passed"):
        raise RuntimeError("address-response canary did not unlock discovery")
    ratio = 512 / 32
    seconds = float(run["elapsed_seconds"]) * ratio
    payload = {
        "status": "address_response_coreset_discovery_resource_estimate",
