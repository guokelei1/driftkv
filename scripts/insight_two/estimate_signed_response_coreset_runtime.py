#!/usr/bin/env python3
"""Seal a conservative 512-user runtime estimate from the focused canary."""
...
def main() -> None:
    ...
    if not run.get("passed") or not gate.get("discovery_launch_gate_passed"):
        raise RuntimeError("signed-response canary did not unlock discovery")
    # Histories and checkpoints are fixed costs per rank, so pure 16x scaling
    # would overestimate. Keep that conservative upper bound for the <30 min gate.
    ratio = 512 / 32
    seconds = float(run["elapsed_seconds"]) * ratio
    from insight_two.common import sha256_file

    payload = {
        "status": "signed_response_coreset_discovery_resource_estimate",
