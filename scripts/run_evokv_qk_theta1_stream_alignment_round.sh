...
SOURCE_CONFIG="configs/evokv_foundation/qk_theta1_stream_alignment_two_gpu_v0.json"
ROUND_ROOT="results/foundation_model/qk_theta1/qk_theta1_stream_alignment_20260806_round1"
...
if [[ -f "$FROZEN_CONFIG" ]]; then
    if ! cmp --silent "$SOURCE_CONFIG" "$FROZEN_CONFIG"; then
        if [[ -f "$RESULT" || -s "$EVALUATION_LOG" ]]; then
            echo "frozen QK alignment config changed after execution started" >&2
            exit 1
        fi
...
if [[ -f "$RESULT" ]]; then
    ...
    echo "QK theta1 stream alignment result is already complete and valid"
    exit 0
fi
...
if missing:
    raise FileNotFoundError(f"required QK alignment inputs are absent: {missing}")
if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
    raise RuntimeError("QK alignment round requires exactly GPU0/GPU1")
...
sha256sum \
    ...
    "scripts/run_evokv_qk_theta1_stream_alignment_round.sh" \
...
if [[ -f "$INPUT_HASHES" && -s "$EVALUATION_LOG" ]]; then
    cmp --silent "$HASH_CANDIDATE" "$INPUT_HASHES" || {
        echo "QK alignment inputs changed after execution started" >&2
        exit 1
    }
else
...
if [[ "${EVOKV_PREFLIGHT_ONLY:-0}" == "1" ]]; then
    echo "QK theta1 stream alignment preflight completed"
    exit 0
fi
...
gate = result["quality"]["evaluation"]["alignment_gate"]
manifest = {
