...
PROTOCOL = "evokv_qk_theta2_core_heavy_round_plan_v0"
CANDIDATES = {
...
def _write_frozen(path: Path, value: dict[str, object]) -> None:
    ...
    if path.is_file():
        if path.read_text() != payload:
            raise FileExistsError(f"frozen QK core-heavy config differs: {path}")
        return
...
def _validate_plan(plan_path: Path, plan: dict[str, object]) -> None:
    ...
    if (
        ...
        raise ValueError("QK theta2 core-heavy plan differs")
    ...
    if (
        ...
        raise ValueError(f"QK core-heavy frozen input differs: {plan_path}")
...
def _relevance_config(
    ...
    return {
        ...
        **_common(plan, candidate),
        ...
        "origin": {
            "kind": "core_heavy_search_candidate",
...
def materialize(
    ...
    if only_candidate is not None and only_candidate not in CANDIDATES:
        raise ValueError(f"unknown QK core-heavy candidate: {only_candidate}")
    if canary_records_per_rank < 0:
        raise ValueError("QK core-heavy canary size differs")
        ...
        training_config = {
            ...
            **_common(plan, name),
            ...
            "exploration": {
                ...
                "axis": candidate["axis"],
                "optimizer_partition": {
                    "dense": "complete HSTU core including direct k_proj/v_proj",
                    "projection": "owner-side E4096-to-H1536 input projection",
                    "embedding": "sparse item table used by history input and candidate scoring",
                },
            },
            "training": {
