...
SOURCE_CONFIG="configs/evokv_foundation/qk_theta1_stream_sensitivity_two_gpu_v0.json"
ROUND_ROOT="results/foundation_model/qk_theta1/qk_theta1_stream_sensitivity_20260806_round1"
...
if [[ -f "$FROZEN_CONFIG" ]]; then
    cmp --silent "$SOURCE_CONFIG" "$FROZEN_CONFIG" || {
        echo "frozen QK sensitivity config differs from source config" >&2
        exit 1
    }
else
...
if [[ -f "$RESULT" ]]; then
    python scripts/validate_evokv_qk_theta1_stream_sensitivity.py \
        --config "$FROZEN_CONFIG" 2>&1 | tee -a "$VALIDATION_LOG"
    echo "QK theta1 stream sensitivity result is already complete and valid"
    exit 0
fi
...
if missing:
    raise FileNotFoundError(f"required QK sensitivity inputs are absent: {missing}")
if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
    raise RuntimeError("QK sensitivity round requires exactly GPU0/GPU1")
...
sha256sum \
    "$FROZEN_CONFIG" \
    "src/hstu_kvcache/streaming/qk_sensitivity_runner.py" \
    "src/hstu_kvcache/streaming/qk_stream_version.py" \
    "scripts/evaluate_evokv_qk_theta1_stream_sensitivity.py" \
    "scripts/validate_evokv_qk_theta1_stream_sensitivity.py" \
    "scripts/run_evokv_qk_theta1_stream_sensitivity_round.sh" \
...
if [[ "${EVOKV_PREFLIGHT_ONLY:-0}" == "1" ]]; then
    echo "QK theta1 stream sensitivity preflight completed"
    exit 0
fi
