...
from search_kuairand_long_context_compiled_rank import (
    PROTOCOL as RANK_PROTOCOL,
)
from search_kuairand_long_context_compiled_rank import (
    evaluate_local,
)
...
PROTOCOL = "kuairand_long_context_4plus12_attention_weighted_search_v1"
DEFAULT_RANK_RESULT = (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rank-result", default=DEFAULT_RANK_RESULT)
...
def mix_name(mix: float) -> str:
    return f"compiled_attention_mix_{mix:.2f}"


@torch.inference_mode()
def fit_attention_family(
    current: HSTU,
    old: HSTU,
    samples: list[dict],
    ...
    importance_chunks: list[list[torch.Tensor]] = [
        [] for _ in current.blocks
    ]
    valid_tokens = 0
    torch.cuda.synchronize(device)
    started = time.perf_counter()
    for _, full_cpu, prefix_cpu, _ in eval_batches(
        samples,
        args.seq_len,
        args.batch_size,
    ):
        ...
        fresh_state = capture_layerwise_state(
            current,
            full["item_ids"],
            full["behaviors"],
            full["time_deltas"],
        ...
        rows = torch.arange(full["item_ids"].shape[0], device=device)
        latest = (full["lengths"] - 1).clamp_min(0)
        prefix_width = prefix["item_ids"].shape[1]
        for layer, features in enumerate(old_state.normed_states):
            block = current.blocks[layer]
            fresh_k = fresh_state.kv.k[layer][:, :prefix_width]
            fresh_v = fresh_state.kv.v[layer][:, :prefix_width]
            residual = torch.cat(
                (
                    fresh_k - cheap.k[layer],
                    fresh_v - cheap.v[layer],
                ),
                dim=-1,
            )
            query_state = fresh_state.normed_states[layer][rows, latest]
            query = block.attn.q_proj(query_state).view(
                len(rows),
            ...
            activated = block.attn._activate(logits)
            importance = activated.square().mean(dim=1).sqrt()
            importance = importance * valid
            mean = importance.sum(dim=1) / prefix["lengths"].clamp_min(1)
            importance = importance / mean.clamp_min(
                torch.finfo(importance.dtype).eps
            ).unsqueeze(1)
            importance = importance.clamp_max(args.attention_weight_cap)
            feature_chunks[layer].append(features[valid].cpu())
            residual_chunks[layer].append(residual[valid].cpu())
            importance_chunks[layer].append(importance[valid].cpu())
    base = compile_projection_cache_adapter(current)
    family_weights = {mix: [] for mix in args.attention_mixes}
    family_biases = {mix: [] for mix in args.attention_mixes}
    sampled_tokens = []
    importance_stats = []
    for layer, (features, residuals, importances) in enumerate(
        zip(
            feature_chunks,
            residual_chunks,
            importance_chunks,
        ...
        importance_stats.append(
            {
                ...
                "p95": float(torch.quantile(importance, 0.95)),
        ...
        for mix in args.attention_mixes:
            weight = (1.0 - mix) + mix * importance
            weight_sum = weight.sum().clamp_min(
                torch.finfo(weight.dtype).eps
            )
            feature_mean = (x * weight.unsqueeze(1)).sum(dim=0) / weight_sum
            residual_mean = (y * weight.unsqueeze(1)).sum(dim=0) / weight_sum
            centered_x = x - feature_mean
            centered_y = y - residual_mean
            weighted_x = centered_x * weight.sqrt().unsqueeze(1)
            weighted_y = centered_y * weight.sqrt().unsqueeze(1)
            gram = weighted_x.transpose(0, 1) @ weighted_x / weight_sum
            cross = weighted_x.transpose(0, 1) @ weighted_y / weight_sum
            ...
            correction = torch.linalg.solve(
                gram + args.ridge * scale * identity,
                cross,
            )
            correction_bias = residual_mean - feature_mean @ correction
            family_weights[mix].append(base.weights[layer] + correction)
            family_biases[mix].append(base.biases[layer] + correction_bias)
    family = {
        mix_name(mix): CompiledCacheAdapter(
            weights=torch.stack(family_weights[mix]),
            biases=torch.stack(family_biases[mix]),
    ...
    return family, {
        ...
        "attention_importance_per_layer": importance_stats,
        "attention_mixes": args.attention_mixes,
...
def broadcast_family(
    ...
    names = [mix_name(mix) for mix in args.attention_mixes]
...
def select_mix(pairs: list[dict], args: argparse.Namespace) -> dict:
    candidates = []
    for mix in args.attention_mixes:
        name = mix_name(mix)
        candidates.append(
    ...
    return {
        "rule": (
            ...
            "version probe cohorts; break ties by top-100 overlap then lower weighting"
        ),
        "candidates": candidates,
        "selected_attention_mix": selected["attention_mix"],
    }


def validate_args(args: argparse.Namespace) -> None:
    if len(set(args.attention_mixes)) != len(args.attention_mixes):
        raise ValueError("attention mixes must be unique")
    if args.attention_mixes != sorted(args.attention_mixes):
        raise ValueError("attention mixes must be sorted")
    if min(args.attention_mixes) < 0 or max(args.attention_mixes) > 1:
        raise ValueError("attention mixes must be in [0, 1]")
    if args.attention_weight_cap <= 1:
        raise ValueError("attention weight cap must exceed one")
    if args.cache_versions != [0, 4, 10]:
        raise ValueError("attention search freezes cache versions")
    if args.current_version != 11 or args.base_days != 4:
        raise ValueError("attention search freezes theta11 under 4+12")
    frozen = {
    ...
    if changed:
        raise ValueError(f"frozen attention search settings changed: {changed}")
...
def main() -> None:
    ...
    try:
        if runtime.device.type != "cuda":
            raise ValueError("attention search requires CUDA")
        if runtime.world_size != 2:
            raise ValueError("attention search freezes evaluation at two GPUs")
        seed_everything(0)
        rank_result = json.loads(Path(args.rank_result).read_text())
        if (
            rank_result.get("protocol") != RANK_PROTOCOL
            or rank_result.get("status") != "design_search_complete"
        ):
            raise ValueError("rank search result is invalid or incomplete")
        if rank_result["design"]["selection"]["selected_rank"] != 512:
            raise ValueError("attention search requires the rank-512 incumbent")
        args.ridge = float(rank_result["pairs"][0]["fit"]["ridge"])
        ...
        if runtime.is_primary:
            selection = select_mix(pairs, args)
            selected_name = mix_name(
                selection["selected_attention_mix"]
            )
            result = {
                "protocol": PROTOCOL,
                "status": "design_search_complete",
                "source_training_result": args.training_result,
                "source_rank_result": args.rank_result,
                "prepared_data": {
                ...
                "design": {
                    "family": (
                        "HSTU attention-use-weighted compiled full-affine transport"
                    ),
                    "online_operator_invariant": (
                        "every weighting compiles to the same one-pass affine projection"
                    ),
                    "ridge": args.ridge,
                    "selection": selection,
                },
            ...
            print(
                json.dumps(
                    {
                        "output": args.output,
                        "selected_attention_mix": selection[
