...
SOURCE_CONFIG="configs/evokv_foundation/qk_theta1_reuse_recompute_full_catalog_two_gpu_v1.json"
DATA_CONFIG="configs/evokv_foundation/qk_theta0_theta7_stream_data_v0.json"
ROUND_ROOT="results/foundation_model/qk_theta1/qk_theta1_reuse_recompute_full_catalog_20260805_round1"
...
TRAINING_LOG="$ROUND_ROOT/training_full_catalog.log"
...
if [[ -f "$FROZEN_CONFIG" ]]; then
    cmp --silent "$SOURCE_CONFIG" "$FROZEN_CONFIG" || {
        echo "frozen theta1 full-catalog config differs from source config" >&2
        exit 1
    }
else
...
if [[ -f "$RESULT" ]]; then
    python scripts/summarize_evokv_qk_reuse_recompute.py \
        --config "$FROZEN_CONFIG" 2>&1 | tee -a "$SUMMARY_LOG"
    python scripts/validate_evokv_qk_full_catalog_tuning.py \
        --config "$FROZEN_CONFIG" 2>&1 | tee -a "$VALIDATION_LOG"
    echo "QK theta1 full-catalog tuning measurement is already complete and valid"
    exit 0
fi
...
if final_checkpoint.exists():
    raise FileExistsError(f"target checkpoint already exists: {final_checkpoint}")
...
sha256sum \
    ...
    "scripts/train_evokv_qk_theta1_full_catalog.py" \
    "scripts/summarize_evokv_qk_reuse_recompute.py" \
    "scripts/validate_evokv_qk_full_catalog_tuning.py" \
    "scripts/run_evokv_qk_theta1_full_catalog_round.sh" > "$INPUT_HASHES"

if [[ "${EVOKV_PREFLIGHT_ONLY:-0}" == "1" ]]; then
    echo "QK theta1 full-catalog handoff preflight completed"
    exit 0
fi
...
artifacts = {
    ...
    "metric_table_json": {
        "path": config["outputs"]["metric_table_json"],
        "sha256": digest(config["outputs"]["metric_table_json"]),
    },
    "metric_table_csv": {
        "path": config["outputs"]["metric_table_csv"],
        "sha256": digest(config["outputs"]["metric_table_csv"]),
    },
    "metric_table_markdown": {
        "path": config["outputs"]["metric_table_markdown"],
        "sha256": digest(config["outputs"]["metric_table_markdown"]),
    },
    "provisional_checkpoint_manifest": {
        "path": str(checkpoint),
        "sha256": digest(checkpoint),
    },
    "frozen_config": {"path": str(config_path), "sha256": digest(config_path)},
    "training_full_catalog_log": {
        "path": str(round_root / "training_full_catalog.log"),
        "sha256": digest(round_root / "training_full_catalog.log"),
    },
    "validation_log": {
