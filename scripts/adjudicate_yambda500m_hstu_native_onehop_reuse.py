...
from evaluate_yambda500m_hstu_native_onehop_reuse_raw import PAIR_PATHS, sha256_file, validate_pair_raw
...
def main() -> None:
    ...
    report = {
        ...
        "reuse_minus_recompute": {
            "paired_harm": paired_harm(uids=np.asarray([row["uid"] for row in current]), labels=targets, reuse_logits=reuse_logits, current_logits=current_logits, namespace=f"d14-onehop:{seal['stage']}"),
