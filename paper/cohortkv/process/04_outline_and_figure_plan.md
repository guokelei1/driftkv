...
| 7 Destination engine | How is a whole target version produced? | waves, program residency, multi-GPU extent placement, destination transaction, manifest. |
| 8 Implementation | What exists today? | PyTorch/Triton runtime and four backend interfaces; status boundaries. |
| 9 Evaluation | What is established, and at what evidence level? | RQ1 opportunity, RQ2 compiler, RQ3 operator/runtime, RQ4 architecture correctness and open gate. |
| 10 Discussion and limitations | When does the approach not apply? | 50% capsule state, simplified HSTU, adaptive seed-0, source/full-cohort/storage gaps. |
...
- v4 destination engine: interface validated, full-cohort speedup open.
