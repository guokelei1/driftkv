...
| A2 | Verified full-affine programs preserve high current-model semantic fidelity on the held-out final users. | Final cost 0.0638/0.0640/0.0641× exact; K/V recovery 0.8865/0.8911/0.9356 for ages 11/7/1. At age 11, signed gap recovery is 98.8% MeanRank/AUC, 90.3% NDCG@100, 88.9% Hit@100. This certificate used in-memory FP32 layerwise state. | Use semantic probes without labels, then repeat the frozen certificate on serialized deployment precision. | Always attach “adaptive seed-0,” disjoint-split, and numeric-representation details. | Treat the 522 users as independent training replications or claim the existing certificate already covers deployed FP16 shards. |
...
| O1 | Full-cohort identical-boundary evaluation is missing. | Stage 0 freezes 682 records, 1,087,785 prefix tokens, a controlled 136/205/341 source mix, common source tier, HBM/DRAM boundaries, capacity preflight, and exact 18-point schema coverage; no current result executes that matrix. | Keep engine performance claim open. | “The full-cohort contract is frozen; measurement is open.” | Fill the table from the blueprint, extrapolate numbers, call the controlled source mix organic, silently omit an infeasible HBM point, or reuse v2 as v4 evidence. |
...
| O3 | Runtime fallback orchestration is missing. | Verified plans serialize ordered residual/structural/exact fallbacks, but v4 currently consumes compiled affine programs and does not dispatch the escalation chain. Stage-0 audit shows p8 also needs 5.83 GiB of separately retained old hidden suffix for its theta0/theta10 scope. | Wire only executable actions and their declared source state into the coordinator; otherwise revise the plan to fall through to exact. | “The plan records ordered fallbacks; automatic dispatch and residual state are open.” | “The destination engine automatically escalates failed cohorts,” or count the hidden suffix as part of the default capsule. |
