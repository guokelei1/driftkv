...
![](images/79dd602895eefb8c2b5f6150e9d5c715c4cfdb77a739c2f925cbbae59e2c3966.jpg)
...
https://doi.org/10.1145/3600006.3613165 the cost per request—of LLM serving systems is becoming more important.

![](images/66fa07a1bb794498bdd1756a4eeb7f49eb88e150d1807332ee93406fb152f6bb.jpg)

<details>
<summary>line</summary>

| Batch size (# requests) | Memory usage (GB)::Existing systems | Memory usage (GB)::vLLM | Throughput (token/s)::Existing systems | Throughput (token/s)::vLLM |
| --- | --- | --- | --- | --- |
| 0 | ~26 | ~26 | ~0.3k | ~0.9k |
| 8 | ~39 | — | — | — |
| 40 | — | ~39 | — | — |
...
The task of language modeling is to model the probability of a list of tokens $( x _ { 1 } , \ldots , x _ { n } )$ . Since language has a natural sequential ordering, it is common to factorize the joint prob ability over the whole sequence as the product of conditional probabilities (a.k.a. autoregressive decomposition [3]):

$$
P (x) = P (x _ {1}) \cdot P (x _ {2} \mid x _ {1}) \dots P (x _ {n} \mid x _ {1}, \dots , x _ {n - 1}). \tag {1}
$$
...
To address this problem, fine-grained batching mechanisms, such as cellular batching [16] and iteration-level scheduling [60], have been proposed. Unlike traditional methods that work at the request level, these techniques operate at the iteration level. After each iteration, completed requests are removed from the batch, and new ones are added. Therefore, a new request can be processed after waiting for a single iteration, not waiting for the entire batch to complete. Moreover, with special GPU kernels, these techniques eliminate the need to pad the inputs and outputs. By reducing the queueing delay and the ineficiencies from padding, the fine-grained batching mechanisms significantly increase the throughput of LLM serving.

![](images/374dbcc7bbdb11baf21bbe272a454867bbd6465f0d7a5f8f1e403392d2c500b6.jpg)
...
![](images/5824b0beb9c74d2b904ed88ef3538b492243227f45448082badba7445498354d.jpg)
...
```mermaid
graph LR
  A["Scheduler"] --> B["KV Cache Manager\nBlock tables"]
  A --> C["Worker 0\nCache Engine\nModel Shard 0"]
  A --> D["Worker 1\nCache Engine\nModel Shard 1"]
  A --> E["Worker N - 1\nCache Engine\nModel Shard N - 1"]
  B --> F["CPU Block Allocator"]
  B --> G["GPU Block Allocator"]
```
</details>
...
![](images/1592c645ca768bc2d8e60eb5703ab6478aff4ee4f75117e317103eb661d6d2b9.jpg)
...
```mermaid
graph LR
  RequestA["Request A"] --> Prompt["Prompt: 'Four score and seven years ago our' Outputs: 'fathers' → 'brought' → ..."]
  Prompt --> Block0["Block 0"]
  Prompt --> Block1["Block 1"]
  Prompt --> Block2["Block 2"]
  Prompt --> Block3["Block 3"]
  Block0 --> LogicalKVBlocks["Logical KV blocks"]
  LogicalKVBlocks --> Block1["Block 1"]
  LogicalKVBlocks --> Block2["Block 2"]
  LogicalKVBlocks --> Block3["Block 3"]
  LogicalKVBlocks --> Block4["Block 4"]
  LogicalKVBlocks --> Block5["Block 5"]
  LogicalKVBlocks --> Block6["Block 6"]
  LogicalKVBlocks --> Block7["Block 7"]
  LogicalKVBlocks --> Block8["Block 8"]
  
  subgraph PhysicalKVBlocks ["Physical KV blocks (on GPU DRAM)"]
    direction TB
  Block0["Block 0"] --> Block1["Block 1"]
  Block1["Block 1"] --> Block2["Block 2"]
  Block2["Block 2"] --> Block3["Block 3"]
  Block3["Block 3"] --> Block4["Block 4"]
  Block4["Block 4"] --> Block5["Block 5"]
  Block5["Block 5"] --> Block6["Block 6"]
  Block6["Block 6"] --> Block7["Block 7"]
  Block7["Block 7"] --> Block8["Block 8"]
...
![](images/407580ce299db0080d291c457a839d32e4a2c8c27bf77526ca4fdd6182731aef.jpg)
...
graph LR
  subgraph Request_A["Request A"]
    Block0["Block 0: Four score and seven"]
    Block1["Block 1: years ago our fathers"]
    Block2["Block 2: brought"]
    Block3["Block 3: —"]
    Block4["Block 4: —"]
    Block5["Block 5: It was the best"]
    Block6["Block 6: —"]
    Block7["Block 7: Four score and seven"]
    Block8["Block 8: —"]
  end

  subgraph Physical_KV_Block["Physical KV blocks"]
  Block0 --> Block1
  Block1 --> Block2
  Block2 --> Block3
  Block3 --> Block4
...
![](images/8a866e52b6450159edb7e066bd6d4f0fc774cb61613592dbdbffd334992ddc85.jpg)
...
graph LR
  Block0["Block 0"] --> Block1["Block 1"]
  Block0 --> Block2["Block 2"]
  Block1 --> Block3["Block 3"]
  Block2 --> Block4["Block 4"]
  Block3 --> Block5["Block 5"]
  Block3 --> Block6["Block 6"]
  Block4 --> Block7["Block 7"]
  Block4 --> Block8["Block 8"]
  Block5 --> Block9["Block 9"]
  Block6 --> Block10["Block 10"]
  Block7 --> Block11["Block 11"]
  Block8 --> Block12["Block 12"]
```
</details>

Figure 9. Beam search example.

Unlike parallel decoding, beam search facilities sharing not only the initial prompt blocks but also other blocks across diferent candidates, and the sharing patterns dynamically change as the decoding process advances, similar to the process tree in the OS created by compound forks. Fig. 9 shows how vLLM manages the KV blocks for a beam search example with � = 4. Prior to the iteration illustrated as the dotted line, each candidate sequence has used 4 full logical blocks. All beam candidates share the first block 0 (i.e., prompt). Candidate 3 digresses from others from the second block. Candidates 0-2 share the first 3 blocks and diverge at the fourth block. At subsequent iterations, the top-4 probable candidates all originate from candidates 1 and 2. As the original candidates 0 and 3 are no longer among the top candidates, their logical blocks are freed, and the reference counts of corresponding physical blocks are reduced. vLLM frees all physical blocks whose reference counts reach 0 (blocks 2, 4, 5, 8). Then, vLLM allocates new physical blocks (blocks 9-12) to store the new KV cache from the new candidates. Now, all candidates share blocks 0, 1, 3; candidates 0 and 1 share block 6, and candidates 2 and 3 further share block 7.
...
![](images/243ebfb19fd13d7afa06e8a8facc7ceabc0dce4d43aeb67d08ce40b1e1f8e032.jpg)
...
```mermaid
graph LR
  subgraph Sequence_A["Sequence A\nPrompt"]
    direction TB
    TranslateEnglish["Translate English to French:\n&quot;sea otter&quot; => &quot;loutre de mer&quot;\n&quot;peppermint&quot; => &quot;menthe poivrée&quot;\n&quot;plush girafe&quot; => &quot;girafe en peluche&quot;<br>  end<br><br>  subgraph Sequence_B[&quot;Sequence B\nPrompt"]
    direction TB
    TranslateEnglish["Translate English to French:\n&quot;sea otter&quot; => &quot;loutre de mer&quot;\n&quot;peppermint&quot; => &quot;menthe poivrée&quot;\n&quot;plush girafe&quot; => &quot;girafe en peluche&quot;<br>  end<br><br>  subgraph Task_Input[&quot;Task input"]
    TranslateEnglish
    "cheese" => 
    "I love you" => 
  end
  ...
  subgraph Sequence_B["Sequence B\nLLM output"]
    direction TB
    "Je t'amie"
  end

  Title1["Shared prefix"] --> Title2["Task output"]
```
</details>
...
vLLM is an end-to-end serving system with a FastAPI [15] frontend and a GPU-based inference engine. The frontend extends the OpenAI API [34] interface, allowing users to customize sampling parameters for each request, such as the maximum sequence length and the beam width �. The vLLM engine is written in 8.5K lines ofPython and 2K lines of C++/CUDA code. We develop control-related components including the scheduler and the block manager in Python while developing custom CUDA kernels for key operations such as PagedAttention. For the model executor, we implement pop ular LLMs such as GPT [5], OPT [62], and LLaMA [52] using

![](images/2d24d42cb45357d35c3eb17f026d03d4704d3b2e03996c5921b4b09446eeaea3.jpg)

<details>
<summary>histogram</summary>

| Bin (Tokens) | Density (Input) | Density (Output) |
| --- | --- | --- |
| 0~100 | ~1.75e-2 | ~0.25e-2 |
| 100~200 | ~0.25e-2 | ~0.20e-2 |
| 200~300 | ~0.10e-2 | ~0.18e-2 |
| 300~400 | ~0.05e-2 | ~0.15e-2 |
| 400~500 | ~0.03e-2 | ~0.12e-2 |
| 500~600 | ~0.02e-2 | ~0.08e-2 |
| 600~700 | ~0.01e-2 | ~0.05e-2 |
| 700~800 | ~0.01e-2 | ~0.03e-2 |
| 800~900 | ~0.01e-2 | ~0.01e-2 |
</details>

(a) ShareGPT

![](images/b64f57fe82cd400c5b1d217873c1ef7a2186adbfd7634d249e435df743753207.jpg)
...
![](images/4c323d85b52685205dc94aa786229ec076f7321e22b99730f95fc08a74c52e6b.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Gray Line (Normalized latency s/token) | Red Line (Normalized latency s/token) | Orange Line (Normalized latency s/token) | Green Line (Normalized latency s/token) | Blue Line (Normalized latency s/token) |
| --- | --- | --- | --- | --- | --- |
| 0.0 | ~0.02 | — | — | — | — |
| 0.1 | ~0.15 | — | — | — | — |
| 0.2 | ~0.30 | — | — | — | — |
...
![](images/580240765d15e2ba8a39e3cc840d56c0363099dff9cc112731ca1c3635011bfb.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Orca (Max) | Orca (Pow2) | Blue Line |
| --- | --- | --- | --- |
| 0.0 | — | — | ~0.05 |
| 0.1 | — | — | ~0.15 |
| 0.2 | ~0.3 | ~0.05 | — |
...
![](images/5ca21b8c5be504c47d278fe8d69dbd6a77bae878031334b757918125762ca0ba.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | vLLM | Oracle |
| --- | --- | --- |
| 0.0 | — | 0.0 |
| 0.1 | — | ~0.2 |
| 0.2 | — | ~0.4 |
...
![](images/f803b3717bf6ae94d99f4c2fd853ba4ce2c357687186d88372ee2ae3dc316707.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Gray (s/token) | Red (s/token) | Orange (s/token) | Green (s/token) | Blue (s/token) |
| --- | --- | --- | --- | --- | --- |
| 0 | 0.05 | — | — | — | — |
| ~1 | 0.15 | — | — | — | — |
| ~2 | 0.35 | — | — | — | — |
...
![](images/34e46fc03fbb61897b1ff2b8ddbec17508fc1bfd51f35917d52fc390a49eb45f.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Red (circle) | Grey (line) | Orange (line) | Green (square) | Blue (circle) |
| --- | --- | --- | --- | --- | --- |
| 0 | ~0.05 | ~0.05 | — | — | — |
| 1 | ~0.15 | ~0.25 | — | — | — |
| 2 | ~0.45 | ~0.75 | — | — | — |
...
![](images/0d85be96676fc828a399d7bbc04bc977b2ca51c68cbc30d65b9ebdbf0b96682d.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Grey (Circle) | Red (Square) | Orange (Triangle) | Blue (Square) | Green (Circle) |
| --- | --- | --- | --- | --- | --- |
| 0.5 | ~0.1 | — | — | — | — |
| 1.0 | ~0.35 | — | — | — | — |
| 1.5 | ~0.6 | — | — | — | — |
...
![](images/e892b053330c8ccb6941f9bfe009895e36cd9db3d93705ec05591ef6aeb7fb99.jpg)

<details>
<summary>bar</summary>

| Category | # Batched requests |
| --- | --- |
| Orca (Max) | 7.00 |
| Orca (Pow2) | 9.81 |
| Orca (Oracle) | 13.62 |
...
![](images/459a13d8bd0b6bc9dbd9f3d986e31671a9bdfe6406bca5d01a3a447633d25832.jpg)
...
![](images/b437df62803ef495856c8ecbf33037d234e4ed90979515bce13bc42b44bef3ce.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Red (s/token) | Orange (s/token) | Green (s/token) | Blue (s/token) |
| --- | --- | --- | --- | --- |
| ~1.2 | 0.05 | — | — | — |
| ~1.5 | 0.15 | — | — | — |
| ~6.0 | — | 0.03 | — | — |
...
![](images/c7b3d7bb856df909566f60b2033b186f89854415a8984e0d245c12e826eab1aa.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Orca (Pow2) | Orca (Oracle) | Blue Series |
| --- | --- | --- | --- |
| 0 | ~0.05 | — | — |
| 0.2 | ~0.1 | — | — |
| 0.4 | ~0.3 | — | — |
...
![](images/84945269ca61fda3e3991516bbd2e251b5d0723722c66d949c5f89370acfe63a.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | vLLM (Blue line) | vLLM (Orange line) | vLLM (Green line) | vLLM (Red line) |
| --- | --- | --- | --- | --- |
| 0.2 | — | — | — | ~0.05 |
| 0.4 | — | — | — | ~0.1 |
| 0.6 | — | — | — | ~0.35 |
...
![](images/5c50f6290b5cd18e49f093313609c21f5fcb66a42a778e524271c231d9af745f.jpg)
...
![](images/e2168d2a97de6a3cfe924bc7f8bb06057884ef709b982f88260461e0eed9201b.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Red (x) | Orange (triangle) | Green (square) | Blue (circle) |
| --- | --- | --- | --- | --- |
| 0.2 | ~0.15 | — | — | — |
| 0.4 | ~0.95 | — | — | — |
| 0.8 | — | ~0.05 | — | — |
...
![](images/837ab8a650fe214617102a7ee89524158150e66e3f4eac0b8be9e1ba150d285a.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Red (circle) | Orange (triangle) | Green (square) | Blue (circle) |
| --- | --- | --- | --- | --- |
| 0.1 | ~0.1 | — | — | — |
| 0.2 | ~0.2 | — | — | — |
| 0.5 | — | ~0.05 | — | — |
...
On the ShareGPT dataset, vLLM can sustain 1.7×–2.7× higher request rates compared to Orca (Oracle) and 2.7×–8× compared to Orca (Max), while maintaining similar latencies. This is because vLLM’s PagedAttention can eficiently manage the memory usage and thus enable batching more requests than Orca. For example, as shown in Fig. 13a, for OPT-13B vLLM processes 2.2× more requests at the same time than Orca (Oracle) and 4.3× more requests than Orca (Max). Compared to FasterTransformer, vLLM can sustain up to 22× higher request rates, as FasterTransformer does not utilize a fine-grained scheduling mechanism and ineficiently manages the memory like Orca (Max).
...
![](images/753a5f56e6b58bfee271d78ec970bac396cc547cde3dc853709b7dbc90ba3f83.jpg)

<details>
<summary>bar</summary>

| # Output sequences | Memory saving (%) |
| --- | --- |
| 2 | 6.09 |
| 4 | 8.53 |
| 6 | 9.79 |
...
![](images/27f7dbb4fa73a4a6777c79878bfe062b8e0d585673af32cd45e1a897165d18e0.jpg)

<details>
<summary>bar</summary>

| Beam width | Memory saving (%) |
| --- | --- |
| 2 | 37.56 |
| 4 | 53.13 |
| 6 | 55.16 |
...
![](images/71192fcfe22bb72f7e25b434bd2025305b021147de0c611a89d15e3ba8a8706b.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Orca (Oracle) (Normalized latency (s/token)) | vLLM (Normalized latency (s/token)) |
| --- | --- | --- |
| 0 | ~0.02 | ~0.02 |
| 25 | ~0.04 | ~0.03 |
| 30 | ~0.41 | ~0.05 |
...
![](images/10608156d2dd906e45f63d1c4303c22c3abdffed9e9b0840216d98e4f855ad3e.jpg)

<details>
<summary>line</summary>

| Request rate (req/s) | Orca (Max) | Orca (Pow2) | Orca (Oracle) | vLLM |
| --- | --- | --- | --- | --- |
| 0.1 | ~0.03 | ~0.03 | ~0.03 | ~0.03 |
| 0.2 | ~0.03 | ~0.03 | ~0.03 | ~0.03 |
| 0.3 | ~0.04 | ~0.04 | ~0.04 | ~0.03 |
| 0.4 | ~0.05 | ~0.06 | ~0.06 | ~0.04 |
| 0.5 | ~0.15 | ~0.18 | ~0.18 | ~0.05 |
| 0.6 | ~0.95 | ~0.98 | ~0.85 | ~0.07 |
| 0.7 | — | — | — | ~0.12 |
| 0.8 | — | — | — | ~0.21 |
| 0.9 | — | — | — | ~0.43 |
</details>
...
![](images/0336f5bb8bca497e6368efe3a4464b326f1fc96d4c83cd3f4c565f497be6a5d7.jpg)  
...
![](images/46c4088666ad285af257cfd5dc7c4f81a5eb97843d2a90889ea454e9baee77b9.jpg)

<details>
<summary>line</summary>

| Block size | Recompute (ms) | Swap in (ms) | Swap out (ms) | Swap in + out (ms) |
| --- | --- | --- | --- | --- |
| 1 | ~36 | ~82 | ~56 | ~138 |
| 2 | ~37 | ~54 | ~36 | ~90 |
| 4 | ~37 | ~37 | ~26 | ~61 |
...
![](images/e327b142b3f2197ecce9d1eca6fa0a0c3cfadf6e8f7339fb06b070724185d037.jpg)

<details>
<summary>line</summary>

| Block size | Recompute (stoken) | Swap (stoken) |
| --- | --- | --- |
| 1 | 2.0 | 2.55 |
| 2 | 1.65 | 1.95 |
| 4 | 1.65 | 1.75 |
...
This paper proposes PagedAttention, a new attention algorithm that allows attention keys and values to be stored in non-contiguous paged memory, and presents vLLM, a high-throughput LLM serving system with eficient memory management enabled by PagedAttention. Inspired by operating systems, we demonstrate how established tech niques, such as virtual memory and copy-on-write, can be adapted to eficiently manage KV cache and handle various decoding algorithms in LLM serving. Our experiments show that vLLM achieves 2-4× throughput improvements over the state-of-the-art systems.
...
[38] LMSYS ORG. 2023. Chatbot Arena Leaderboard Week 8: Introduc ing MT-Bench and Vicuna-33B. https://lmsys.org/blog/2023-06-22- leaderboard/.  
...
[45] Haichen Shen, Lequn Chen, Yuchen Jin, Liangyu Zhao, Bingyu Kong, Matthai Philipose, Arvind Krishnamurthy, and Ravi Sundaram. 2019. Nexus: A GPU cluster engine for accelerating DNN-based video anal ysis. In Proceedings ofthe 27th ACM Symposium on Operating Systems Principles. 322–337.  
[46] Ying Sheng, Lianmin Zheng, Binhang Yuan, Zhuohan Li, Max Ryabinin, Daniel Y Fu, Zhiqiang Xie, Beidi Chen, Clark Barrett, Joseph E Gon zalez, et al. 2023. High-throughput Generative Inference of Large  
